...
void VBGO::Init(ID3D11Device* _GD, IEffectFactory* _EF, Game* _game)
{
	GameObject::Init(_GD, _EF, _game);
}


void VBGO::VBGOInit(ID3D11Device* _GD)
{
	//set up all static stuff

	//default vertex shader
	ID3DBlob* pVertexShaderBuffer = NULL;
	HRESULT hr = CompileShaderFromFile(Helper::charToWChar("../Shaders/shader.fx"), "VS", "vs_4_0_level_9_1", &pVertexShaderBuffer);
	_GD->CreateVertexShader(pVertexShaderBuffer->GetBufferPointer(), pVertexShaderBuffer->GetBufferSize(), NULL, &s_pVertexShader);

	//default pixel shader
	ID3DBlob* pPixelShaderBuffer = NULL;
	hr = CompileShaderFromFile(Helper::charToWChar("../Shaders/shader.fx"), "PS", "ps_4_0_level_9_1", &pPixelShaderBuffer);
