...
void VBSnail::Tick(GameState* _GS)
{
	VBGO::Tick(_GS);
}

void VBSnail::Init(ID3D11Device* _GD, IEffectFactory* _EF, Game* _game)
{
	...
	string useFilename = "../Models/" + m_filename + ".txt";
	...
	m_numPrims = 2 * (width - 1) * (m_sections - 1);
	...
	Matrix rotTrans = Matrix::CreateRotationY(m_shellRot);
	Matrix stepTrans = Matrix::CreateTranslation(0.0f, m_step, 0.0f);
	Matrix scaleTrans = Matrix::CreateScale(m_shellScale);
	...
	Matrix* transforms = new Matrix[m_sections];
	...
	//two triangles for each point scale, turn ad step up
	int vert = 0;
	Vector3 calcPos;
	...
	for (int i = 0; i < (m_sections - 1); i++)
	{
		...
		{
			vertices[vert].Color = Color::Lerp(m_col1, m_col2, (float)i / (float)m_sections);
			calcPos = Vector3(baseLine[j].x, baseLine[j].y, 0.0f);
			vertices[vert++].Pos = Vector3::Transform(calcPos, transforms[i]);

			vertices[vert].Color = Color::Lerp(m_col1, m_col2, (float)(i + 1) / (float)m_sections);
			calcPos = Vector3(baseLine[j].x, baseLine[j].y, 0.0f);
	...
	hr = CompileShaderFromFile(Helper::charToWChar("../Shaders/shader.fx"), "PS2", "ps_4_0_level_9_1", &pPixelShaderBuffer);
	_GD->CreatePixelShader(pPixelShaderBuffer->GetBufferPointer(), pPixelShaderBuffer->GetBufferSize(), NULL, &m_pPixelShader);
}

void VBSnail::Load(ifstream& _file)
{
	GameObject::Load(_file);
	StringHelp::String(_file, "FILENAME", m_filename);
	StringHelp::Int(_file, "SECTIONS", m_sections);
	StringHelp::Float(_file, "SHELL SCALE", m_shellScale);
	StringHelp::Float(_file, "SHELL ROT", m_shellRot);
	m_shellRot = XM_PI * m_shellRot / 180.0f;
	StringHelp::Float(_file, "STEP", m_step);
	StringHelp::Float4(_file, "COL1", m_col1.x, m_col1.y, m_col1.z, m_col1.w);
	StringHelp::Float4(_file, "COL2", m_col2.x, m_col2.y, m_col2.z, m_col2.w);
}